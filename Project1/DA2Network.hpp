...
class DA2Network {
public:
  DA2Network(const char *network_path) : env(ORT_LOGGING_LEVEL_WARNING, "da2") {
    ...
    // ---- critical fix: disable optimizations ----
    opts_.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_DISABLE_ALL);
    opts_.SetIntraOpNumThreads(1);
    opts_.DisableMemPattern();

    // set up the Ort session
    session_ = new Ort::Session(env, network_path, opts_);
  }
  ...
  DA2Network(const char *network_path,
    ...
    : env(ORT_LOGGING_LEVEL_WARNING, "da2") {
 ...
 private:
  ...
  char input_names_[256];
  char output_names_[256];
